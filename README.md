...
## ★해야할일★
    ...
    //가장 최근 해야할 일~~ 정리
    //파일을 delete메서드로 삭제하면, ole 폴더를 지우고, ole테이블도 지우는 코드 추가
    //한컴파일 추출 코드 추가 -> 이미지와 같은 기타 파일들은 추출하는데 성공, MS는 문서들은 형태가 달라서 좀 고민해 봐야함
    //한컴 안에 있는 한컴은 테스트 안해봄
    //.OLE라는 파일의 이름이 OLE객체 데이터 파일임. 걸러내는 코드 구현하기(지금은 그냥 모든 객체가 추출된) -> 추가완료
    //.OLE에서 WordDocument나 기타 OFFICE 엔트리가 바로 있으면 문서로 저장, 바로 Ole10Native있으면 기타 파일들 처리
    // 특이하게 HWP에 HWP삽입은 안됨
    //97-03버전은 동일한 문제가 발생(헤더 생성 어려움), 및 03이후 버전은 데이터 부족현상 발생..
    //덮어쓰는건 어떻게는 해볼만 할것 같은데(이게 된다면, doc에서 doc추출이 가능함), 03이후 버전은.....
    //hwpx로 추가할거면 추가하기
    //FileUtil클래스 분할
